...
class UpdateCourseRequest extends FormRequest
{
    ...
    public function rules(): array
    {
        ...
        $typeOfModes = $this->input('type_of_modes', []);
        if (!is_array($typeOfModes)) {
            $typeOfModes = $typeOfModes ? [$typeOfModes] : [];
        }
        
        $selectedModeNames = Mode::whereIn('id', $typeOfModes)
            ->pluck('name')
        ...
        foreach ($billingCycles as $cycle) {
            if (in_array('online', $selectedModeNames)) {
                ...
                $billingRules['online_features_names'] = ['sometimes', 'required', 'array'];
            ...
            if (in_array('in person', $selectedModeNames)) {
                ...
                $billingRules['in_person_features_names'] = ['sometimes', 'required', 'array'];
                $billingRules["in_person_features_names.*"] = ['string', 'min:50', 'max:500'];
            }
        }

        return array_merge([
            'name' => ['sometimes', 'required', 'string', 'max:100', Rule::unique('courses', 'name')->ignore($courseId)],

            'subject_ids' => ['sometimes', 'required', 'array'],
            'subject_ids.*' => ['integer', 'exists:subjects,id'],

            'type_of_modes' => ['sometimes', 'required', 'array'],
            'type_of_modes.*' => ['integer', 'exists:modes,id'],

            'location_ids' => ['sometimes', 'required', 'array'],
            'location_ids.*' => ['integer', 'exists:locations,id'],

            'features_names' => ['sometimes', 'required', 'array'],
            'features_names.*' => ['string', 'min:50', 'max:500'],

            'acdemic_year_id' => ['sometimes', 'required', 'integer', 'exists:acdemic_years,id'],

            'course_image' => ['sometimes', 'nullable', 'image', 'max:10240'],
            'description' => ['sometimes', 'required', 'string', 'min:500', 'max:10000'],
        ], $billingRules);
    }
